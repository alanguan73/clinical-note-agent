...
4. [整体架构](#4-整体架构)  
5. [端到端业务流程](#5-端到端业务流程)  
6. [证据包与多 Agent 路由](#6-证据包与多-agent-路由)  
7. [技术框架](#7-技术框架)  
8. [数据契约](#8-数据契约)  
...
| Agent | `document_type` | 首期 |
|-------|-----------------|------|
| **住院文书 Agent** | `inpatient_admission`（入院记录） | **必交付**（见 [附录 B](#27-附录-b住院入院记录-schema)） |
| 住院文书 Agent | `progress_note`（病程记录） | 首期建议至少支持 1 种病程模板 |
| 门诊病历 Agent | `outpatient` | 可并行；Schema 结构同住院「字段壳」，章节子集 |
| 出院小结 Agent | `discharge` | 二期或首期尾声 |
| 病案首页 Agent | `front_sheet` | 二期 |

**科室**：首期试点儿科、胸外科；其他科室 `department_profile` 配置齐全即用，未命中则 `general` fallback + `warning`。
...
```mermaid
flowchart TB
  subgraph client [客户端 / 集成方]
    UI[展示 / 编辑 / 冲突选择 / 回滚]
    TPL_PAGE[模板配置页 院方已有]
  end
  subgraph gateway [接入层]
    API[BFF API / 鉴权 / 限流 / SSE]
  end
  subgraph orch [编排层]
    ROUTER[文书路由]
    PIPE[流水线状态机]
  ...
  subgraph llm [推理层 可替换]
    CLOUD[公有云 LLM]
    LOCAL[私有化 vLLM/TGI 等]
  end
  subgraph data [数据层]
    PG[(PostgreSQL)]
    VDB[(向量库)]
  ...
  TPL_PAGE -->|template JSON| UI
  UI <--> API
  API --> ROUTER --> PIPE
  PIPE --> REV
  PIPE --> INGEST --> EXTRACT --> NORM --> MERGE --> PATCH
  MERGE --> RAG
  PATCH --> RAG
  PATCH --> RULE
  PATCH -.-> CLOUD
...
**设计要点**：编排与数据面稳定；LLM 经统一 `LLMClient`（OpenAI-compatible）；模板内容**由调用方传入**，本服务只消费与版本记录。
...
sequenceDiagram
  ...
  participant S as BFF/编排
  participant L as LLM
  participant R as RAG/规则

  D->>C: 选择模板 JSON
  C->>S: generate(template, evidence, stream=true)
  S-->>C: SSE template_loaded 全文模板
  par 规则填检验
    S-->>C: section_patch 辅助检查等
  and 流式 ASR final 句
    C->>S: asr_final 累积文本
    S-->>C: physician_verbatim_patch
  and 分节 LLM
    S->>L: patch section_key
    alt 成功
      L-->>S: patches
      S-->>C: section_patch
    else 超时
      S->>L: 重试 最多3次
      alt 仍失败
        S-->>C: section_failed + degraded
      end
    end
  end
  S->>R: 增量+全文合规
  S-->>C: compliance + revision_created + done
  D->>C: 保存草稿 含未解决冲突
```
...
```mermaid
flowchart LR
  EB[EvidenceBundle + template JSON]
  EB --> R{document_type}
  R -->|outpatient| A1[门诊 Agent]
  R -->|inpatient_*| A2[住院 Agent]
  R -->|discharge| A3[出院 Agent]
  A1 --> P[department_profile]
  A2 --> P
  A3 --> P
```

**科室**：`department_id` 加载 profile（检索 filter、增量规则、词表），**不**按科室拆独立 Agent 进程。
...
**全局参数**：`temperature=0`；固定 `prompt_version` 写入 `provenance`；**仅输出 JSON**（解析失败则重试，不计入业务成功）。
...
输出 Schema 见产品字段定义（chief_complaint_candidates、history_of_present_illness、physician_dictation_segments 等）。
```

### 9.2 检验 / 检查非结构化抽取

```text
你是“检验检查结构化抽取模块”。只从输入文本抽取已出现的项目，禁止编造。

硬规则：
1) 不得给出临床诊断结论。
2) 含糊表述写入 ambiguous_findings 并引用原文。
3) 仅输出 JSON。

输入：来源 {{source_type}}，锚点 {{anchors}}，文本 {{lab_or_imaging_text}}

输出：labs[]、imaging[]、ambiguous_findings[]
```

### 9.3 术语规范化（并行字段，不覆盖 verbatim）

```text
你是“医学术语规范化建议模块”。不得新增事实，不得把推测改为确定。

输入：{{phrases_json}}

输出 mappings[]：preferred_term、status=coded|uncoded|ambiguous。
展示默认以 verbatim 为准；normalized 供医师可选查看。
```

### 9.4 证据合并与冲突标注

```text
你是“证据一致性检查模块”。只合并与标注冲突，不做诊断与治疗建议。

合并优先级：接口结构化 > 病历JSON > 报告文本 > ASR口述

输出：
- facts_merged
- conflicts[]（field、structured、dictation、resolution=pending）
- missing_critical[]
```
...
```text
根据 facts_compact 生成检索查询 JSON：queries[]、filters{department, document_type}。
不包含患者标识，不输出医学结论。
```
...
```text
你是“模板病历修订模块”。在既定模板 sections 上，用 facts_merged 与 physician_dictation 做填充/补丁。

硬规则：
1) 一次只处理章节：{{section_key}}。
2) physician_dictation verbatim 优先写入；与模板冲突时以口述为准，记入 patch_log。
3) 仅能用 facts_merged 与 physician_dictation 填充；禁止编造。
4) 不删除未被事实否定的模板安全句；否定须有 evidence_quote。
5) 输出 JSON：
{
  "patches": [{"section_key","op":"replace|append|fill_placeholder","text","source_refs"}],
  "patch_log": [{"section_key","before","after","reason":"fact|physician_dictation|template_default"}],
  "gaps": [],
...
规则引擎产出 errors/warnings；LLM 仅可选将 code 转为 human_readable，**不得新增事实**。
...
### 9.9 再生成：自然语言章节路由（section_intent_router）

```text
你是“病历再生成章节路由模块”。根据医师自然语言指令，判断需要修改的病历章节，不生成病历正文。

硬规则：
1) 输入包含：user_message、document_type、template.sections[].key 列表、当前 draft_note 各节标题摘要（每节取前80字）。
2) 输出仅 JSON：
{
  "target_section_keys": ["present_illness", "..."],
  "confidence": 0.0-1.0,
  "ambiguous_sections": [{"key":"...", "reason":"..."}],
  "needs_clarification": false,
  "clarification_question": null
}
3) 若无法唯一确定章节且 confidence<0.7：needs_clarification=true，并给出 clarification_question（中文一句）。
4) 不得修改 locked_sections 列表中的章节；若 user_message 明确要求改锁定节，在 ambiguous_sections 说明“该节已锁定”。
5) 不得编造医学事实；只做路由。

输入：
- user_message：{{user_message}}
- document_type：{{document_type}}
- available_section_keys：{{section_keys_json}}
- locked_sections：{{locked_sections_json}}
- draft_note_digest：{{draft_note_digest_json}}
```

路由结果 `confidence>=0.7` 且 `needs_clarification=false` 时，仅对 `target_section_keys` 调用 9.6；否则 SSE 推送 `clarification_required` 事件。
...
| 工具 | 职责 |
|------|------|
| `facts.merge` | 合并与 conflicts 生成 |
| `rag.search_policy` | 院规检索 |
| `validate.schema` | JSON Schema |
| `validate.rules` | 病案质控规则包 |
| `revision.create` / `revision.activate` | 版本与回滚 |
| `section.route_intent` | 再生成自然语言找节（9.9） |

模板加载由调用方 JSON 完成，可不设 `template.load` MCP。
...
| 性能 | 模板 T0 不调 LLM；检验规则填槽；分节并行 patch |
...
1. 冻结 **住院入院记录** Schema（附录 B）+ `EvidenceBundle` + `revision` API  
2. 实现 `template_loaded` + 分节 SSE + 节级重试  
3. `conflicts` 双显与 resolve API  
4. 规则 + 草稿保存 + `note_status`（draft/signed）  
5. 再生成：`section_intent_router`（9.9）+ `locked_sections` 默认策略  
6. 儿科/胸外 **住院** 金样例与回归  
7. 门诊/病程/出院扩展；二期可选 9.8  
...
时间、检验表等**规则填槽**；仅叙述性连接句等调 LLM；医生口述**规则映射**，失败进 `unmapped_physician_text`。
...
医师**无需**传 `target_section_keys`；服务端流程：

1. 调用 **9.9 section_intent_router**，输入 `user_message` + 模板节列表 + 当前 `draft_note` 摘要。  
2. 若 `needs_clarification=true` → 返回 `clarification_required`（SSE/HTTP），**不 patch**。  
3. 若 `confidence>=0.7` → 仅对 `target_section_keys` 调用 9.6（跳过 `locked_sections`）。  
4. 生成新 `revision_id`，流式推送各节 `section_patch`。
...
sequenceDiagram
  participant D as 医生
  participant S as 服务
  participant R as section_intent_router
  participant P as template_patch 9.6

  D->>S: regenerate(user_message)
  S->>R: 9.9 路由
  alt 需澄清
    R-->>S: needs_clarification
    S-->>D: clarification_required
  else 已确定章节
    R-->>S: target_section_keys
    loop 每节未锁定
      S->>P: patch section
      P-->>D: section_patch SSE
    end
    S-->>D: revision_created + done
  end
```
...
| 场景 | 处理 |
|------|------|
| 指令含明确章节名（「现病史」「查体」） | 映射到 `section_key`，confidence 高 |
| 指令含内容线索（「白细胞」「车祸时间」） | 结合 `draft_note_digest` 推断节 |
| 多节都可能 | `ambiguous_sections`，`needs_clarification=true` |
| 目标节 locked | 跳过并在响应 `warnings` 说明 |
| 指令要求新增院外资料 | `trigger=add_material`，合并 `evidence_delta` 后重新 merge，再 patch |

### 26.3 评测

首期科内集：**≥50 条**真实口语修改指令，自动找节 **准确率 ≥90%**（人工标注 `target_section_keys` 为金标准）。
...
| `ROUTER_CLARIFICATION` | 422 | 无法自动找节，需澄清 |
...
| v0.3 | 2026-05-16 | 安全专章（默认策略）；附录 D 金样例 D-1～D-6 |
